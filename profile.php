<?php
    ...
    if(isset($_POST['login'])) {
        ...
        $userType = $_POST['user_type']; // Added line to get the selected user type

        if($userType == 'admin') {
            $query = mysqli_query($con, "SELECT ID FROM tbladmin WHERE UserName='$username' AND Password='$password'");
        } elseif($userType == 'student') {
            $query = mysqli_query($con, "SELECT ID FROM student WHERE StudentUserName='$username' AND StudentPassword='$password'");
        }
        ...
        if($ret > 0) {
            ...
            if($userType == 'admin') {
                header('location: admin_dashboard.php'); // Change to the appropriate admin dashboard page
            } elseif($userType == 'student') {
                header('location: student_dashboard.php'); // Change to the appropriate student dashboard page
            }
        } else {
    ...
    // Rest of your code for profile update
    // Note: You might need to adjust this part based on your actual tables and fields.
    // The assumption here is that both admin and student tables have 'AdminName', 'MobileNumber', 'Email' fields.
    if (strlen($_SESSION['avmsaid']) == 0) {
        ...
        if(isset($_POST['update'])) {
            ...
            $query = mysqli_query($con, "UPDATE student SET AdminName='$AName', MobileNumber='$mobno', Email='$email' WHERE ID='$adminid'");
